...
public class LibraryPluginService {
   ...
   */
  public List<LibraryPlugin> getAllPlugins(final String email) throws PluginException {
    ...
    try {
      ...
      throw new PluginException("Failed to load all plugins", error);
    }
  }
  ...
  public LibraryPlugin createPlugin(final String pluginLanguage, final String pluginFilename)
    ...
    try {
      ...
      throw new PluginException("Failed to create plugin", error);
    }
  }
   ...
   */
  @Transactional
  public void deletePlugin(final long id) throws PluginException {
    log.debug("Deleting plugin: id={}", id);
    this.libraryPluginRepository.delete(this.doLoadPlugin(id));
  }

  private LibraryPlugin doLoadPlugin(final long id) throws PluginException {
    final LibraryPlugin result = this.libraryPluginRepository.getById(id);
    if (result == null) throw new PluginException("No such plugin: id=" + id);
    return result;
  }
}
