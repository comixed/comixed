...
function usage() {
  ...
  echo "Usage: dbrestore.sh FILENAME -d [DATABASE] -j [URL] [OPTIONS]"
  if [[ ! -z ${message} ]]; then
    echo ""
    echo "${message}"
  fi
...
java -cp $LIBDIR/h2*jar org.h2.tools.RunScript -url jdbc:h2:file:${DATABASE} ${OPTIONS} -script ${BACKUP_FILE} -options compression zip
