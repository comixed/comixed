...
import org.comixed.scrapers.ComicVineQueryWebRequest;
import org.comixed.scrapers.WebRequest;
...
public class ComicVineQueryForVolumesAdaptorTest {
  ...
  @InjectMocks private ComicVineQueryForVolumesAdaptor adaptor;
  @Mock private WebRequestProcessor webRequestProcessor;
  @Mock private ObjectFactory<ComicVineQueryWebRequest> webRequestFactory;
  @Mock private ComicVineQueryWebRequest webRequest;
  @Mock private ComicVineVolumesResponseProcessor queryResult;
  @Captor private ArgumentCaptor<List<ScrapingVolume>> comicVolumeList;
  @Mock private List<ComicVineVolumeQueryCacheEntry> queryEntries;
  @Mock private ComicVineVolumeQueryCacheEntry queryEntry;
  @Captor private ArgumentCaptor<ComicVineVolumeQueryCacheEntry> queryEntryCaptor;
  @Mock private ComicVineVolumeQueryCacheRepository queryRepository;
  ...
  public void testExecuteWebRequestProcessorRaisesException()
    ...
    Mockito.when(queryRepository.findBySeriesNameOrderBySequence(Mockito.anyString()))
        .thenReturn(null);
    ...
    try {
      ...
      Mockito.verify(queryRepository, Mockito.times(1))
          .findBySeriesNameOrderBySequence(TEST_VOLUME_NAME);
  ...
  public void testExecuteResultProcessorRaisesException()
    ...
    Mockito.doNothing().when(webRequest).setSeriesName(Mockito.anyString());
    Mockito.when(webRequestProcessor.execute(Mockito.any(WebRequest.class)))
        .thenReturn(TEST_RESPONSE_CONTENT_TEXT);
    ...
    try {
      ...
      Mockito.verify(webRequest, Mockito.times(1)).setSeriesName(TEST_VOLUME_NAME);
      Mockito.verify(webRequestProcessor, Mockito.times(1)).execute(webRequest);
      Mockito.verify(queryResult, Mockito.times(1))
          .process(comicVolumeList.getValue(), TEST_RESPONSE_CONTENT);
    }
  }
  ...
  public void testExecuteEntryInDatabase() throws ComicVineAdaptorException, WebRequestException {
    ...
    Mockito.when(queryEntry.getAgeInDays())
        .thenReturn(ComicVineVolumeQueryCacheEntry.CACHE_TTL - 1);
  ...
  @Test
  public void testExecuteEntryInDatabaseSkipCache()
  ...
  @Test
  public void testExecuteEntryInDatabaseExpired()
      throws ComicVineAdaptorException, WebRequestException {
    Mockito.when(queryRepository.findBySeriesNameOrderBySequence(Mockito.anyString()))
        .thenReturn(queryEntries);
  ...
  public void testExecuteWithMultiplePages() throws WebRequestException, ComicVineAdaptorException {
    ...
    Mockito.verify(queryRepository, Mockito.times(1)).save(queryEntryCaptor.getValue());
    Mockito.verify(queryResult, Mockito.times(2))
        .process(comicVolumeList.getValue(), TEST_RESPONSE_CONTENT);
  }
}
