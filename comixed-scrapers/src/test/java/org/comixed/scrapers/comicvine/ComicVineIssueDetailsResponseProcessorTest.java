...
@SpringBootTest(
    classes = {
      ComicVineIssueDetailsResponseProcessor.class,
...
public class ComicVineIssueDetailsResponseProcessorTest {
  ...
  public void testProcess() throws ComicVineAdaptorException {
    Mockito.doNothing().when(comic).clearCharacters();
    Mockito.doNothing().when(comic).addCharacter(Mockito.anyString());
    Mockito.doNothing().when(comic).clearLocations();
    Mockito.doNothing().when(comic).addLocation(Mockito.anyString());
    Mockito.doNothing().when(comic).clearStoryArcs();
    ...
    Mockito.doNothing().when(comic).setComicVineURL(Mockito.anyString());
    ...
    Mockito.doNothing().when(comic).setCoverDate(coverDate.capture());
    ...
    Mockito.verify(comic, Mockito.times(1)).addCharacter("X-23");
    Mockito.verify(comic, Mockito.times(1)).clearLocations();
    Mockito.verify(comic, Mockito.times(1)).addLocation("Genosha");
    Mockito.verify(comic, Mockito.times(1)).addLocation("Xavier Institute");
    Mockito.verify(comic, Mockito.times(1)).addLocation("Genosha");
    Mockito.verify(comic, Mockito.times(1)).clearStoryArcs();
    ...
    Mockito.verify(comic, Mockito.times(1))
        .setDescription(
            "<p><em>In the wake of an unprecedented assault on Atlantis, the X-Men must react and recover...while at the mercy of a world that grows more hostile to mutants every day, and a foe who is determined to keep it that way!</em></p><h4>List of covers and their creators:</h4><table data-max-width=\"true\"><thead><tr><th scope=\"col\">Cover</th><th scope=\"col\">Name</th><th scope=\"col\">Creator(s)</th><th scope=\"col\">Sidebar Location</th></tr></thead><tbody><tr><td>Reg</td><td>Regular Cover</td><td>Jenny Frison</td><td>1</td></tr><tr><td>Var</td><td>Cosmic Ghost Rider Vs. Variant Cover</td><td>Jamal Campbell</td><td>2</td></tr><tr><td>Var</td><td>Legacy Headshot Variant Cover (Trinary)</td><td>Travis Charest &amp; Tamra Bonvillain</td><td>3</td></tr></tbody></table>");
    Mockito.verify(comic, Mockito.times(1)).setSeries("X-Men: Red");
    Mockito.verify(comic, Mockito.times(1)).setIssueNumber("8");
    Mockito.verify(comic, Mockito.times(1)).setTitle("The Hate Machine Part 8: Global Hatred");
    Mockito.verify(comic, Mockito.times(1)).setCoverDate(coverDate.capture());
  }
}
