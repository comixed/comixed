...
@SpringBootTest(
    classes = {
      ComicVineIssueResponseProcessor.class,
...
public class ComicVineIssueResponseProcessorTest {
  ...
  public void testProcess() throws ComicVineAdaptorException {
    ...
    assertEquals(660635, result.getId());
    assertEquals("38", result.getIssueNumber());
    assertEquals("2018-04-30", result.getCoverDate());
    assertEquals(
        "https://comicvine.gamespot.com/api/image/original/6291970-38.jpg", result.getCoverUrl());
    assertEquals("Harley Quinn", result.getVolumeName());
    assertEquals(92750, result.getVolumeId());
  }
}
