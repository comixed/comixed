...
public class OPDSController
{
    ...
    @ResponseBody
    @RequestMapping(value = "/opds-comics", method = RequestMethod.GET,produces =
    ...
    @ResponseBody
    @RequestMapping(value = "/opds-comics/all",
    ...
    public OPDSFeed getAllLists() throws ParseException
    {
        ...
        return new OPDSNavigationFeed("/opds-comics/all?groupByFolder=true", "Comics - ", this.readingListService.getReadingListsForUser(authentication.getName()));
    }

    @ResponseBody
    @RequestMapping(value = "/opds-comics/{id}",
            method = RequestMethod.GET, produces =
            MediaType.APPLICATION_XML_VALUE)
    @CrossOrigin
    public OPDSFeed getList(@PathVariable("id") long id) throws ParseException, NoSuchReadingListException
    {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        return new OPDSNavigationFeed("/opds-comics/" + id, "Comics - ", this.readingListService.getReadingListForUser(authentication.getName(), id));
    }


    @RequestMapping(value = "/opds-comics/feed/comics/{id}/download/{filename}", method = RequestMethod.GET)
    @CrossOrigin
    ...
    @RequestMapping(value = "/opds-comics/feed/comics/{id}/{index}/{maxWidth}", method = RequestMethod.GET)
    @CrossOrigin
    public ResponseEntity<byte[]> getImageInComicByIndex(@PathVariable("id") long id, @PathVariable("index") int index, @PathVariable("maxWidth") int maxWidth) throws IOException
    {
                ...
                .getPageCount()))
        {
            ...
            if (maxWidth != 0 && maxWidth < image.getWidth()) {
                ...
                ToolkitImage scaled = (ToolkitImage) image.getScaledInstance(maxWidth, -1, Image.SCALE_SMOOTH);

                Image temporary = new ImageIcon(scaled).getImage();
                BufferedImage buffered = new BufferedImage(scaled.getWidth(), scaled.getHeight(), BufferedImage.TYPE_INT_RGB);
