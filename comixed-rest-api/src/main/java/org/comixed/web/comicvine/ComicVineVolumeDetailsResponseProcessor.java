...
public class ComicVineVolumeDetailsResponseProcessor {
    ...
    public String process(byte[] content,
        ...
        try {
            ...
            comic.setVolume(jsonNode.get("results")
                                    .get("start_year")
                                    .asText());
            comic.setSeries(jsonNode.get("results")
                                    .get("name")
                                    .asText());
            result = jsonNode.get("results")
                             .get("publisher")
                             .get("id")
                             .asText();
        }
        catch (IOException error) {
            throw new ComicVineAdaptorException("Invalid response content",
                                                error);
        }
