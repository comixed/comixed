...
public class UserControllerTest {
    ...
    public void testAfterPropertiesSet()
        ...
        Mockito.when(userService.findRoleByName("READER"))
               .thenReturn(TEST_ROLE_READER);
        Mockito.when(userService.findRoleByName("ADMIN"))
               .thenReturn(TEST_ROLE_ADMIN);

        controller.afterPropertiesSet();

        assertSame(TEST_ROLE_READER,
                   controller.readerRole);
        assertSame(TEST_ROLE_ADMIN,
                   controller.adminRole);

        Mockito.verify(userService,
                       Mockito.times(1))
               .findRoleByName("READER");
        Mockito.verify(userService,
                       Mockito.times(1))
    ...
    public void testSetUserProperty()
        ...
        Mockito.when(userService.setUserProperty(Mockito.anyString(),
                                                 Mockito.anyString(),
                                                 Mockito.anyString()))
               .thenReturn(user);

        ComiXedUser result = controller.setUserProperty(authentication,
                                                        TEST_PROPERTY_NAME,
                                                        TEST_PROPERTY_VALUE);
    ...
    public void testSetUserPropertyForInvalidUser()
        ...
        try {
            controller.setUserProperty(authentication,
                                       TEST_PROPERTY_NAME,
                                       TEST_PROPERTY_VALUE);
        }
        finally {
    ...
    public void testGetCurrentUser()
        ...
        Mockito.doNothing()
               .when(user)
               .setAuthenticated(Mockito.anyBoolean());
    ...
    public void testUpdatePassword()
        ...
        Mockito.when(authentication.getName())
               .thenReturn(TEST_EMAIL);
        Mockito.when(userService.setUserPassword(Mockito.anyString(),
                                                 Mockito.anyString()))
               .thenReturn(user);

        final ComiXedUser result = controller.updatePassword(authentication,
                                                             TEST_PASSWORD);
        ...
        Mockito.verify(userService,
                       Mockito.times(1))
               .setUserPassword(TEST_EMAIL,
                                TEST_PASSWORD);
    }

    @Test(expected = ComiXedUserException.class)
    public void testSetUserEmailInvalidUser()
            throws
            ComiXedUserException {
        Mockito.when(authentication.getName())
               .thenReturn(TEST_EMAIL);
        Mockito.when(userService.setUserEmail(Mockito.anyString(),
                                              Mockito.anyString()))
               .thenThrow(ComiXedUserException.class);

        try {
            controller.setUserEmail(authentication,
                                    TEST_EMAIL);
        }
        finally {
            Mockito.verify(authentication,
    ...
    public void testSetUserEmail()
        ...
        final ComiXedUser result = controller.setUserEmail(authentication,
                                                           TEST_EMAIL);
    ...
    public void testUpdateUserRemoveAdmin()
        ...
        Mockito.doNothing()
               .when(user)
               .addRole(Mockito.any(Role.class));
        Mockito.when(userService.save(Mockito.any(ComiXedUser.class)))
               .thenReturn(user);
