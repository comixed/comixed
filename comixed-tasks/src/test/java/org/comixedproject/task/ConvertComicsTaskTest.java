...
public class ConvertComicsTaskTest {
  ...
  @InjectMocks private ConvertComicsTask task;
  @Mock private TaskService taskService;
  @Mock private ObjectFactory<ConvertComicTaskEncoder> convertComicTaskEncoderObjectFactory;
  ...
  public void testStartTask() throws TaskException, ComicException {
    ...
    Mockito.when(convertComicTaskEncoderObjectFactory.getObject())
        .thenReturn(convertComicTaskEncoder);
    Mockito.when(taskService.save(taskArgumentCaptor.capture())).thenReturn(savedPersistedTask);

    task.startTask();

    for (int index = 0; index < idList.size(); index++)
      Mockito.verify(comicService, Mockito.times(1)).getComic(idList.get(index));
    Mockito.verify(convertComicTaskEncoderObjectFactory, Mockito.times(idList.size())).getObject();
