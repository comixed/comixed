...
export class ComicFileListComponent implements OnInit, OnDestroy {
    ...
    this.contextMenu = [
      {
        ...
        command: () => this.importAdaptor.select_comic_files(this.comicFiles),
        disabled:
      ...
      },
      {
        ...
        command: () =>
          this.importAdaptor.deselect_comic_files(this.selectedComicFiles),
        visible:
  ...
  startImporting(messageKey: string, ignoreMetadata: boolean): void {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.importAdaptor.start_importing(false, ignoreMetadata);
      }
    });
  }
  ...
  toggleComicFileSelection(comicFile: ComicFile): void {
    if (this.selectedComicFiles.includes(comicFile)) {
      this.importAdaptor.deselect_comic_files([comicFile]);
    } else {
      this.importAdaptor.select_comic_files([comicFile]);
    }
  }
}
