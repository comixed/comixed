...
import { duplicatesReducer, initial_state } from 'app/reducers/duplicates.reducer';
import * as DupesActions from 'app/actions/duplicate-pages.actions';
import { DUPLICATE_PAGE_1, DUPLICATE_PAGE_2 } from 'app/models/comics/duplicate-page.fixtures';
...
describe('duplicatesReducer', () => {
  ...
  describe('when fetching pages', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesFetchPages());
    });
    ...
    it('resets the deleted pages count', () => {
      expect(state.pages_deleted)
        .toEqual(0);
    });

    it('resets the undeleted pages count', () => {
      expect(state.pages_undeleted)
        .toEqual(0);
    });
  });
  ...
  describe('when setting the duplicate pages', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesSetPages({
        duplicate_pages: [
          DUPLICATE_PAGE_1,
          DUPLICATE_PAGE_2
        ]
    ...
    it('sets the pages to those received', () => {
      expect(state.pages)
        .toEqual([DUPLICATE_PAGE_1, DUPLICATE_PAGE_2]);
    });

    it('sets the set of hashes to those received', () => {
      expect(state.hashes)
        .toEqual([DUPLICATE_PAGE_1.hash]);
    });

    it('sets the pages by hash for what was received', () => {
      expect(state.pages_by_hash.size)
        .toEqual(1);
      expect(state.pages_by_hash.get(DUPLICATE_PAGE_1.hash))
        .toEqual([DUPLICATE_PAGE_1, DUPLICATE_PAGE_2]);
    });
  ...
  describe('when deleting all of the duplicate pages', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesDeleteAll({ hash: DUPLICATE_PAGE_1.hash }));
    });
  ...
  describe('when receiving the deleted pages for a hash', () => {
    beforeEach(() => {
      state.pages = [{ ...DUPLICATE_PAGE_1, deleted: false }, { ...DUPLICATE_PAGE_2, deleted: false }];
      state = duplicatesReducer(state,
        new DupesActions.DuplicatePagesDeletedForHash({ hash: DUPLICATE_PAGE_1.hash, count: 17 }));
    });
  ...
  describe('when undeleting all pages', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesUndeleteAll({ hash: DUPLICATE_PAGE_1.hash }));
    });
  ...
  describe('when receiving the the undelete pages response', () => {
    beforeEach(() => {
      state.pages = [{ ...DUPLICATE_PAGE_1, deleted: true }, { ...DUPLICATE_PAGE_2, deleted: true }];
      state = duplicatesReducer(state,
        new DupesActions.DuplicatePagesUndeletedForHash({ hash: DUPLICATE_PAGE_1.hash, count: 23 }));
    });
  ...
  describe('when marking a page hash as blocked', () => {
    beforeEach(() => {
      state = duplicatesReducer(state,
        new DupesActions.DuplicatePagesBlockHash({ hash: DUPLICATE_PAGE_1.hash }));
    });
  ...
  describe('when marking a page hash as unblocked', () => {
    beforeEach(() => {
      state = duplicatesReducer(state,
        new DupesActions.DuplicatePagesUnblockHash({ hash: DUPLICATE_PAGE_1.hash }));
    });
  ...
  describe('when receiving a blocked page response', () => {
    beforeEach(() => {
      state.pages = [
        { ...DUPLICATE_PAGE_1, blocked: false }, { ...DUPLICATE_PAGE_2, blocked: false }];
      state = duplicatesReducer(state,
        new DupesActions.DuplicatePagesBlockedHash({ hash: DUPLICATE_PAGE_1.hash, blocked: true }));
    });
  ...
  describe('when showing pages with a hash', () => {
    beforeEach(() => {
      state.pages_by_hash.set(DUPLICATE_PAGE_1.hash, [DUPLICATE_PAGE_1, DUPLICATE_PAGE_2]);
      state = duplicatesReducer(state,
        new DupesActions.DuplicatePagesShowComicsWithHash({ hash: DUPLICATE_PAGE_1.hash }));
    });

    it('sets the hash in use', () => {
      expect(state.current_hash)
        .toEqual(DUPLICATE_PAGE_1.hash);
    });

    it('sets the current duplicate pages', () => {
      expect(state.current_duplicates)
        .toEqual([DUPLICATE_PAGE_1, DUPLICATE_PAGE_2]);
    });
  });
  ...
  describe('when showing all duplicate pages', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesShowAllPages());
    });
  ...
  describe('when deleting a page', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesDeletePage({ page: PAGE_1 }));
    });
  ...
  describe('when undeleting a page', () => {
    beforeEach(() => {
      state = duplicatesReducer(state, new DupesActions.DuplicatePagesUndeletePage({ page: PAGE_1 }));
    });
