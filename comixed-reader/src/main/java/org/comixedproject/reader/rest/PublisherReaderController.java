...
@RestController
@Log4j2
public class PublisherReaderController {
  public static final String GET_PUBLISHERS_URL = API_ROOT + "/collections/publishers";
  @Autowired private DirectoryReaderService directoryReaderService;

  /**
   * Returns all publishers. If the unread parameter is present then only those publishers with
   ...
   */
  @GetMapping(value = GET_PUBLISHERS_URL, produces = MediaType.APPLICATION_JSON_VALUE)
  @PreAuthorize("hasRole('READER')")
  @Timed(value = "comixed.reader.library.get-publishers")
  public LoadDirectoryResponse getPublishers(
    ...
    log.info("Loading publishers: email={} unread={}", email, unread);
    final LoadDirectoryResponse result = new LoadDirectoryResponse();
    result
        .getContents()
        .addAll(directoryReaderService.getAllPublishers(email, unread, GET_PUBLISHERS_URL));
    return result;
  }
   ...
   */
  @GetMapping(
      value = API_ROOT + "/collections/publishers/{publisher}",
      produces = MediaType.APPLICATION_JSON_VALUE)
  @PreAuthorize("hasRole('READER')")
  @Timed(value = "comixed.reader.library.get-series-for-publisher")
  public LoadDirectoryResponse getSeriesForPublishers(
      final Principal principal,
    ...
    result
        ...
        .addAll(
            directoryReaderService.getAllSeriesForPublisher(
                ...
                publisher,
                String.format(
                    "%s/%s/series", GET_PUBLISHERS_URL, ReaderUtil.urlEncode(publisherParam))));
    return result;
  }
}
