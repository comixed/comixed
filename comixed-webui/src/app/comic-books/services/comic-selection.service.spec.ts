...
import { ComicSelectionService } from './comic-selection.service';
import {
...
import { MultipleComicSelectionRequest } from '@app/comic-books/models/net/multiple-comic-selection-request';
...
import { DuplicateComicsSelectionRequest } from '@app/comic-books/models/net/duplicate-comics-selection-request';
import { UnreadComicsSelectionRequest } from '@app/comic-books/models/net/unread-comics-selection-request';
...
describe('ComicSelectionService', () => {
  ...
  const COMIC_IDS = [3.2, 96, 9, 21, 98];
  ...
  let service: ComicSelectionService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.inject(ComicSelectionService);
  ...
  it('can set the selection state by filter', () => {
    ...
    expect(req.request.body).toEqual({
      ...
      searchText: SEARCH_TEXT,
      selected: SELECTED
    } as MultipleComicSelectionRequest);
    req.flush(serverResponse);
  });
  ...
  it('can select comics based on their read state', () => {
    ...
    expect(req.request.body).toEqual({
      selected: SELECTED,
      unreadOnly: UNREAD_ONLY
    } as UnreadComicsSelectionRequest);
    req.flush(serverResponse);
  });
