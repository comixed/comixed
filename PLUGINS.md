...
A plugin must provide methods that return the plugin's **name**, it's **version**, and the **list of properties** that
must be configured
...
| **comicIds**           | The selected comic book ids, as am instance of ```List<Long>``` |
| **comicService**       | Provides access to the comic table.                             |
