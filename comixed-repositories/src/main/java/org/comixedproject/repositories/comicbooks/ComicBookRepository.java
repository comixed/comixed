...
public interface ComicBookRepository extends JpaRepository<ComicBook, Long> {
   ...
   */
  @Query(
      "SELECT new org.comixedproject.model.collections.Publisher(d.publisher, count(c), count(DISTINCT d.series)) FROM ComicDetail d, ComicBook c WHERE LENGTH(c.comicDetail.publisher) > 0 AND d.publisher = c.comicDetail.publisher AND c.id = d.comicBook.id GROUP BY d.publisher")
