...
public interface ComiXedUserRepository extends JpaRepository<ComiXedUser, Long> {
  ...
  @Query(
      "SELECT new org.comixedproject.model.net.user.ComicsReadStatistic(d.publisher, COUNT(d)) FROM Comic d WHERE d.publisher IS NOT NULL AND d.comicId IN (SELECT rcb FROM ComiXedUser u INNER JOIN u.readComicBooks rcb WHERE u.email = :email) GROUP BY d.publisher")
