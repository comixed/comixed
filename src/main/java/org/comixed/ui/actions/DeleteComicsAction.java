 ...
 */
@Component
public class DeleteComicsAction extends AbstractAction
{
    ...
    {
        int[] rows = comicTableView.getSelectedRows();
        List<Comic> comics = new ArrayList<>();

        for (int row : rows)
        {
            comics.add(comicSelectionModel.getComic(row));
        }

        logger.debug("Prompting the user to delete " + comics.size() + " comics");

        if (JOptionPane.showConfirmDialog(mainFrame,
                                          messageSource.getMessage("dialog.confirm.delete.message", new Object[]
                                          {comics.size()}, Locale.getDefault()),
                                          messageSource.getMessage("dialog.confirm.delete.title", null,
                                                                   Locale.getDefault()),
                                          JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
        {
            boolean deleteFileAsWell = (JOptionPane.showConfirmDialog(mainFrame,
                                                                      messageSource.getMessage("dialog.confirm.delete.include-file.label",
                                                                                               new Object[]
                                                                                               {comics.size()},
                                                                                               Locale.getDefault()),
                                                                      messageSource.getMessage("dialog.confirm.delete.include-file.title",
                                                                                               null,
                                                                                               Locale.getDefault()),
                                                                      JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION);
            {
                logger.debug("Deleting comics...");
                DeleteComicsWorkerTask task = taskFactory.getObject();
        ...
        {
            logger.debug("User declined to delete comics...");
        }
    }
}
